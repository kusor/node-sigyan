...
function decreaseLevel(logger) {
    if (!logger._level || !logger.level()) {
        return console.error('Unkwnown logger object (), skipping.');
    }
    ...
    if (pos === (levels.length - 1)) {
        console.warn('Log level already set to the minimun. ' +
          'Doing nothing.');
    } else {
...
function increaseLevel(logger) {
    ...
    if (pos === 0) {
        console.warn('Log level already set to the maximun. ' +
          'Doing nothing.');
    } else {
        logger.level(levels[pos - 1]);
        console.log('Log level set to ' + levels[pos - 1]);
